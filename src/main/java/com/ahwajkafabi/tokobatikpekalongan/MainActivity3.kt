...
import com.ahwajkafabi.tokobatikpekalongan.www.listmovierecyclerview.network.InitRetrofit3
...
class MainActivity3 : AppCompatActivity() {
                    ...
                    override fun onResponse(call: Call<ResponseArticles>, response: Response<ResponseArticles>) {
                        if (response.isSuccessful) {
                            ...
                            if (response.body()?.status!!) {
                                ...
                                rvListEncyclopedia.adapter = MainAdapter(this@MainActivity3, response.body()?.data!!)
                            } else {
    ...
    fun showArticles() {
        ...
        //TODO: Ambil data dari API
        InitRetrofit3().getInstance().requestGetArticles3().enqueue(object :
            Callback<ResponseArticles3> {
            override fun onFailure(call: Call<ResponseArticles3>, t: Throwable) {
                t.printStackTrace()
                hideLoading()
            }

            override fun onResponse(call: Call<ResponseArticles3>, response: Response<ResponseArticles3>) {
                if (response.isSuccessful) {
                    hideLoading()
                    if (response.body()?.status!!) {
                        dataArticles = response.body()?.data!!
                        rvListEncyclopedia.adapter = MainAdapter3(this@MainActivity3, dataArticles)
                    } else {
